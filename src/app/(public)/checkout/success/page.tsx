...
export default async function CheckoutSuccessPage({ searchParams }: { searchParams: { orderId: string } }) {
  ...
  const order = await getOrderByIdUseCase(Number(orderId));
        ...
        <CardContent>
          <div className="space-y-4">
            <p className="text-center text-gray-600">
              Cảm ơn bạn đã mua hàng tại <span className="font-bold text-secondary-400">GEM</span>. Mã đơn hàng của bạn là:
            </p>
